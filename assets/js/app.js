...
function applyDocumentDefaults() { const manifest = state.data?.manifest || {}; const defaults = manifest.defaults || manifest.theme?.defaults || {}; const theme = manifest.theme || {}; const normalizedRegistry = normalizeRegistry(manifest.registry || {}); state.activeVariantId = normalizeActiveVariantId(normalizeVariants(state.data || {}), defaults.variantId || manifest.activeVariantId); state.kanji = defaults.kanji === "shinjitai" ? "shinjitai" : "original"; state.ruby = defaults.ruby !== false; state.writingMode = defaults.writingMode === "vertical" ? "vertical" : "horizontal"; applyThemeTypography(theme); state.font = theme.font?.type === "remote" ? "custom" : (theme.font?.id || theme.font || "serif"); state.fontUrl = theme.font?.type === "remote" ? String(theme.font.url || "") : ""; state.background = validColor(theme.background, "#f5f0e6"); state.paletteBank = normalizedRegistry.activeBank || "default"; state.color = validColor(theme.color, paletteValue(normalizedRegistry, 0, state.paletteBank)); state.remoteFontsAllowed = true; syncControls(); applyAppearance(); render(); savePreferences(); updateStatus("作品既定の表示設定に戻しました"); }
