...
function syncPaletteControls() { const bank = $("palette-bank"); const slot = $("palette-slot"); const color = $("palette-color"); if (!slot || !color) return; const normalized = normalizeRegistry(state.data?.manifest?.registry || {}); if (bank) { bank.replaceChildren(...Object.keys(normalized.banks).map(name => { const option = document.createElement("option"); option.value = name; option.textContent = name; return option; })); state.paletteBank = normalized.banks[state.paletteBank] ? state.paletteBank : normalized.activeBank; bank.value = state.paletteBank; } const slots = normalized.banks[state.paletteBank]?.slots || normalized.palettes; const indices = [...new Set(["0", "1", "2", "3", ...Object.keys(slots || {})])].sort((a, b) => Number(a) - Number(b)); slot.replaceChildren(...indices.map(index => { const option = document.createElement("option"); option.value = index; option.textContent = normalized.banks[state.paletteBank]?.names?.[index] ? `Slot ${index} · ${normalized.banks[state.paletteBank].names[index]}` : `Slot ${index}`; return option; })); const value = slots?.[slot.value] || paletteValue(normalized, Number(slot.value), state.paletteBank); if (validColor(value, "")) color.value = value; }
...
function sourceInput() { if (state.mode !== "source" || !state.data) return; const editor = $("source-editor"); const start = editor.selectionStart; const end = editor.selectionEnd; const result = parseSourceEditorInput(editor.value, currentAdapter()); if (!result.ok) { editor.setCustomValidity(result.error.message); editor.setAttribute("aria-invalid", "true"); setStatus(`Sourceを反映できません: ${result.error.message}`); return; } clearReaderError(); editor.setCustomValidity(""); editor.removeAttribute("aria-invalid"); const record = currentRecord(); state.data = replaceVariantSource(state.data, record.id, result.source); state.nodes = result.document.nodes; if (state.data.titleSource === "first-line") state.data.manifest.title = resolveTitle({ source: result.source }); markDirty({ source: true }); saveDraft(); scheduleHistory(); render(captureScroll()); editor.focus(); editor.setSelectionRange(Math.min(start, editor.value.length), Math.min(end, editor.value.length)); updateStatus(); }
...
function saveDraft() { if (!state.data) return; try { storageSet(draftKey(), JSON.stringify(draftPayload(state.data, titleSourceText(), state.activeVariantId))); } catch { state.storageAvailable = false; setStatus("自動復元用Storageを利用できません。編集は継続できます"); } }
...
function bind() {
  ...
  $("background-color").addEventListener("input", e => { state.background = e.target.value; applyAppearance(); savePreferences(); }); $("text-color").addEventListener("input", e => { state.color = e.target.value; const registry = normalizeRegistry(state.data?.manifest?.registry || {}); const currentBank = registry.banks[state.paletteBank] || registry.banks.default; const banks = { ...registry.banks, [state.paletteBank]: { ...currentBank, slots: { ...currentBank.slots, "0": state.color } } }; state.data.manifest.registry = { ...registry, banks, activeBank: state.paletteBank, palettes: banks.default.slots, paletteNames: banks.default.names }; state.data.manifest.theme = { ...(state.data.manifest.theme || {}), color: state.color }; markDirty({ document: true }); saveDraft(); applyAppearance(); savePreferences(); updateStatus(); }); $("remote-font-toggle").addEventListener("change", async e => { ++fontRequestToken; state.remoteFontsAllowed = e.target.checked; savePreferences(); try { await loadConfiguredFont(); render(); setStatus(state.remoteFontsAllowed ? "外部フォントの自動読込を許可しました" : "外部フォントの自動読込を停止しました"); } catch { render(); setStatus("外部フォントを読み込めませんでした。標準フォントを使用します"); } }); $("font-family").addEventListener("change", async e => { const token = ++fontRequestToken; const selected = e.target.value; try { await loadWebFont(selected); if (token !== fontRequestToken) return; state.font = selected; applyAppearance(); savePreferences(); setStatus("フォントを適用しました"); } catch { if (token === fontRequestToken) setStatus("フォントを読み込めませんでした"); } });
