...
const state = { kana: "historical", kanji: "original", ruby: true, writingMode: "horizontal", size: 20, font: "serif", fontUrl: "", background: "#f5f0e6", color: "#272522", rubyColor: "#716b60", annotations: [], data: null, nodes: [] };
...
function buildReaderDocument() { return { version: 1, content: { text: currentSource(), format: "narou-text" }, meta: { title: $("song-title").textContent, artist: $("song-artist").textContent, description: $("song-description").textContent }, theme: { font: state.font === "custom" ? { type: "remote", url: state.fontUrl } : state.font, background: state.background, color: state.color, rubyColor: state.rubyColor }, defaults: { kana: state.kana, kanji: state.kanji, ruby: state.ruby, writingMode: state.writingMode }, annotations: state.annotations, links: Object.fromEntries([...$("song-links").querySelectorAll("a")].map(link => [link.textContent, link.href])) }; }
function setReaderDocument(documentData, name = "Reader文書") { const content = documentData?.content?.text; if (typeof content !== "string") throw new Error("Reader文書に本文がありません。"); const manifest = { ...(documentData.meta || {}), title: documentData.meta?.title || "Reader文書", content: { format: documentData.content.format || "narou-text" }, defaults: documentData.defaults || {}, theme: documentData.theme || {}, annotations: documentData.annotations || [], links: documentData.links || {} }; state.data = { manifest, historical: { text: content, url: "reader:" }, modern: { text: content, url: "reader:" } }; applyManifest(manifest); applyAppearance(); render(); setStatus(`${name}を読み込みました`); }
...
function bind() {
  ...
  $("reader-shell").addEventListener("wheel", event => { const lyrics = $("lyrics"); if (event.target.closest(".settings, .source-input")) return; if (state.writingMode === "vertical") lyrics.scrollLeft += event.deltaY || event.deltaX; else lyrics.scrollTop += event.deltaY; }, { passive: true });
