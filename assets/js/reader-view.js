import { parseSource, serializeSource } from "./syntax-adapter.js";
...
export function rawText(nodes, range = null) {
  if (!range) return serializeSource({ type: "document", nodes });
  const selected = [];
  let offset = 0;
  ...
  return serializeSource({ type: "document", nodes: selected });
}
