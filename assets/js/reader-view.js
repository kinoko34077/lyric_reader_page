...
export function renderLyrics(element, source, options) {
  ...
  for (const node of nodes) {
    if (node.type === "text") { for (const char of [...node.value]) { const span = document.createElement("span"); span.className = "source-char"; span.dataset.sourceStart = offset; span.dataset.sourceEnd = offset + char.length; span.dataset.sourceRaw = char; span.textContent = char; Object.assign(span.style, annotationStyle(offset, offset + char.length)); fragment.append(span); offset += char.length; } continue; }
    const end = offset + [...node.base].length; const wrapper = document.createElement("span"); wrapper.className = "source-ruby"; wrapper.dataset.sourceStart = offset; wrapper.dataset.sourceEnd = end; wrapper.dataset.sourceRaw = serializeRuby([node]); Object.assign(wrapper.style, annotationStyle(offset, end));
