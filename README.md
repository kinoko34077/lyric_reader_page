...
`npm run test:writer`はWriter Beta Browser Gateです。Readerの自動Mobile Gateとは独立しており、parse成功時の反映、Variant別Source、Draft復元、文書Open Undo、不正入力のCurrent Document保護、Storage書込み不能時の編集継続、Viewer上のStyle / Palette / Glyph / Combine / Outline適用、Ruby Reading部分のScoped Style適用、Presentation解除、既存Presentation内の本文直接置換・キャレット入力、Source→Viewer→SourceのAuthor Source保持をChromiumで確認します。Ruby部分指定は`ruby-range` / `ruby-style`、縁取りは`outline=name`として保存され、解除後も元Sourceへ戻ることを確認します。Writer Beta Gateの失敗は現段階ではReader Pages公開を止めません。
