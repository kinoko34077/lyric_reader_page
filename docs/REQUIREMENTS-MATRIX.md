...
| Writer K1 / K6 | Writer Betaの状態管理 | K6は`sessionStorage`由来のTab固有Draft keyとWriter Beta Gateで検証。K1はDeferred | K1=文書OpenをUndo対象、K6=Tab単位Draft分離。Reader v0.xの完成条件外 | K6は実装済み。K1は現行Readerの文書単位Historyから未拡張 |
