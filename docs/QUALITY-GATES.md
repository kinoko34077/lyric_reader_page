...
- `npm run test:writer`で、ChromiumのWriter Core Browser Gateを単独検証する。確定したAuthor Sourceを起点に、valid Source→Viewer→Writerの正本保持と、parse不能なSourceをViewerへ反映せずCurrent Documentを保護する最小往復を確認する。Writer全体を一つの状態依存シナリオへ連結しないため、Source Editor、Document transaction、WYSIWYG、Tab isolation、Storage故障、不正文Draftの詳細確認は下位Gateとして同じランナーから独立実行し、最後に結果を集約して終了コードへ反映する。WYSIWYGでは既存Presentation内の置換・改行・削除に加え、Rubyを跨ぐsemantic削除と平坦化されたRuby隣接文字の削除を確認する。Presentation Authoringは`npm run test:writer-presentation`、Writer表示面は`npm run test:writer-mobile`で別Context・別jobとして実行する。
...
- Writer modeへの起動、Title / 本文の表示とcontenteditable状態、Ruby、Variant / Ruby切替、設定Panelのviewport内表示と内部scroll、縦書き時のTitle / 本文writing-mode同期、意図しない横overflow、Source→Viewer→Writer往復を確認する。表示DOMがRubyを平坦化した場合の隣接入力と、Portable RubyのCopy→PasteがAuthor SourceへRubyとして復元されることもChromium / WebKit双方で確認する。
