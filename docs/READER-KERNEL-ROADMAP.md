...
`ad0aac3`以降、単一Writer Surface、Source range transaction、動的Syntax解釈、Ruby／IME／Title境界のSource保全、本文Projectionの範囲縮小、Typography 14／20／32px、反復記号2セルadvance、Nishiki-teki未検証状態の明示まで実装・Browser Gate検証済み。通常入力・削除・改行・Paste・CompositionはDOM全体のSource再構築を通らず、既存Projectionの骨格が変わらない場合はテキスト／属性だけを同期する。Writer MobileはChromium Pixel 5相当／WebKit iPhone 13相当でPASSしている。次の未完了は、WYSIWYG編集中のSelection／Caret／Viewportをさらに壊さない部分更新の拡張と、実機iPhone Safari／Native Clipboard／実IME／Nishiki-teki正式Web Font配信／stable保護の外部Release Smokeであり、[`RELEASE-SMOKE.md`](RELEASE-SMOKE.md)へ分離している。Reader Kernelは引き続き`stable`／`reader-v0.1.0`の`925cfc7`を凍結点とする。
